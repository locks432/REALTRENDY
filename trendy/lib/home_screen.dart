...
import 'package:trendy/dream_space_screen.dart';
    ...
    return GestureDetector(
      onHorizontalDragEnd: (details) {
        if (details.primaryVelocity! < 0) {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const ViralDimensionScreen()),
          );
        } else {
          Navigator.push(
            context,
        ...
        body: Stack(
          children: [
            ...
            GridView.count(
                crossAxisCount: 3,
                children: const [
                  UniverseTile(title: 'Moments', icon: Icons.movie, screen: MomentsScreen()),
                  UniverseTile(title: 'Trendy AI', icon: Icons.lightbulb, screen: TrendyAiScreen()),
                  UniverseTile(title: 'Vibe Room', icon: Icons.music_note, screen: VibeRoomScreen()),
                  UniverseTile(title: 'Launchpad', icon: Icons.rocket_launch, screen: LaunchpadScreen()),
                  UniverseTile(title: 'Live Arenas', icon: Icons.sports_soccer, screen: LiveArenasScreen()),
